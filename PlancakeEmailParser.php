...
class PlancakeEmailParser {
  ...
  public function getMessageID()
         {
           if ( (!isset($this->rawFields['message-id'])) || (!count($this->rawFields['message-id'])))
           {
             throw new Exception("Couldn't find the message-id");
           }
           return $this->rawFields['message-id'];
         }

  public function getInReplyTo()
         {
           if ( (!isset($this->rawFields['in-reply-to'])) || (!count($this->rawFields['in-reply-to'])))
           {
             return false;
           }
           return $this->rawFields['in-reply-to'];
         }
  ...
  public function getFrom()
         {
           ...
           $ret = '';
           if ($this->isImapExtensionAvailable) {
             $array = imap_mime_header_decode($this->rawFields['from']);
             foreach ($array as $h) { // subject can span into several lines
               $charset = ($h->charset == 'default') ? 'US-ASCII' : $h->charset;
               $ret .=  iconv($charset, "UTF-8//TRANSLIT", $h->text);
             }
           } else {
  ...
  public function getBody($returnType=self::PLAINTEXT)
         {
           ...
           if (!$detectedContentType)
           {
             ...
             // check if charset is set in the header
             if (isset($this->rawFields['content-type'])){
               if(preg_match('/charset=(.*)/i',$this->rawFields['content-type'], $matches)) {
                 $charset = strtoupper(trim($matches[1], '"'));
               }
             }
           }

           // check if contentTransferEncoding is in the header
           if (trim($contentTransferEncoding) == '' ){
             if (isset($this->rawFields['content-transfer-encoding'])){
